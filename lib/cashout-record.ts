...
export type CashoutRow = {
  ...
  estimated_vnd: string; fee_vnd: string; net_vnd: string; provider: string; status: string;
...
export function serializeCashout(row: CashoutRow, mint: string) {
  return {
    ...
    feeVnd: row.fee_vnd, netVnd: row.net_vnd, provider: row.provider, providerReference: row.provider_reference,
    ...
    beneficiary: row.beneficiary_id ? { id: row.beneficiary_id, bankCode: row.bank_code, accountLast4: row.account_last4, accountHolderMasked: row.account_holder_masked } : null,
