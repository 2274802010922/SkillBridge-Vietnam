...
export function PassportWorkspace() {
  ...
  useEffect(() => { let active = true; fetch("/api/credentials", { cache: "no-store" }).then((response) => response.ok ? response.json() as Promise<{ credentials: Credential[] }> : Promise.reject(new Error("load"))).then((data) => { if (active) setItems(data.credentials); }).catch(() => { if (active) setLoadError(true); }).finally(() => { if (active) setLoading(false); }); return () => { active = false; }; }, []);
