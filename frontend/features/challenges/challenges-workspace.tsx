...
import { WalletTermsSignature } from "../../components/wallet/wallet-terms-signature";
import { parseChallengeContent, type ChallengeContent } from "../../../shared/validation/challenge-content";
...
type Challenge = {
  ...
  id: string; organization_id: string; organization_name: string; reviewer_organization_id: string | null; reviewer_organization_name: string | null; reviewer_organization_kind: "business" | "university" | null; review_mode: "self" | "independent" | null; title: string; brief: string; content_json: string | null; skills_json: string;
  ...
  reward_amount_atomic: string | null; reward_mint: string | null; access_type: "public" | "invite_only"; status: string; closes_at: string | null; can_manage: number;
...
function badgeDescriptionValue(item: Challenge) { try { return (JSON.parse(item.reward_metadata_json) as { description?: string }).description || ""; } catch { return ""; } }
function parseLines(value: string) { return value.split(/\r?\n/).map((item) => item.trim()).filter(Boolean); }
function challengeContent(item: Challenge) { return parseChallengeContent(item.content_json, item.brief); }

export function ChallengesWorkspace({ memberships, reviewerOrganizations }: { memberships: Membership[]; reviewerOrganizations: ReviewerOrganization[] }) {
  ...
  const [title, setTitle] = useState(""); const [brief, setBrief] = useState(""); const [context, setContext] = useState(""); const [objectives, setObjectives] = useState(""); const [deliverables, setDeliverables] = useState(""); const [constraints, setConstraints] = useState(""); const [timeline, setTimeline] = useState(""); const [skills, setSkills] = useState(""); const [reward, setReward] = useState("");
  ...
  const [editingFinancialLocked, setEditingFinancialLocked] = useState(false);
  ...
  function resetForm() {
    setEditingId(null); setEditingFinancialLocked(false); setTitle(""); setBrief(""); setContext(""); setObjectives(""); setDeliverables(""); setConstraints(""); setTimeline(""); setRewardAmount(""); setRewardType(""); setBadgeNameValue(""); setBadgeDescription(""); setRewardSlots("1"); setMinimumScore("0"); setSkills(""); setReward(""); setAccessType(""); setFormStep(1); setOrganizationId(businessMemberships[0]?.organization_id ?? ""); setReviewMode("self"); setReviewerOrganizationId(reviewerOrganizations[0]?.id ?? "");
  }

  function editDraft(item: Challenge) {
    const content = challengeContent(item);
    setEditingId(item.id); setEditingFinancialLocked(Boolean(item.fund_id)); setOrganizationId(item.organization_id);
    setReviewMode(item.review_mode === "independent" ? "independent" : "self"); setReviewerOrganizationId(item.reviewer_organization_id ?? "");
    setTitle(item.title); setBrief(content.summary); setContext(content.context); setObjectives(content.objectives.join("\n")); setDeliverables(content.deliverables.join("\n")); setConstraints(content.constraints.join("\n")); setTimeline(content.timeline); setSkills(parseSkills(item.skills_json).join(", ")); setReward(item.reward);
    setRewardType(item.reward_type); setRewardAmount(item.reward_amount_usdc ?? ""); setRewardSlots(String(item.reward_slots)); setMinimumScore(item.minimum_score); setBadgeNameValue(item.reward_type === "badge" ? badgeName(item) : ""); setBadgeDescription(item.reward_type === "badge" ? badgeDescriptionValue(item) : ""); setAccessType(item.access_type); setFormStep(1); setNotice(t("challenge.editingDraft"));
    window.setTimeout(() => builderRef.current?.scrollIntoView({ behavior: "smooth", block: "start" }), 0);
  }

  async function saveDraft() {
    setBusy(true); setNotice(null);
    const content: ChallengeContent = { summary: brief, context, objectives: parseLines(objectives), deliverables: parseLines(deliverables), constraints: parseLines(constraints), timeline };
    const payload = { action: editingId ? "update_draft" : undefined, organizationId, reviewerOrganizationId: reviewMode === "independent" ? reviewerOrganizationId : organizationId, reviewMode, title, brief, content, skills: skills.split(","), reward, rewardType: rewardType || undefined, badgeName: badgeNameValue || undefined, badgeDescription: badgeDescription || undefined, rewardSlots: Number(rewardSlots) || 1, minimumScore, rewardAmountUsdc: rewardAmount || undefined, accessType };
    const response = await fetch(editingId ? `/api/challenges/${editingId}` : "/api/challenges", { method: editingId ? "PATCH" : "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(payload) });
    const data = await response.json() as { error?: string };
    setNotice(response.ok ? (editingId ? t("challenge.updated") : t("challenge.created")) : data.error ?? t("challenge.actionError"));
    if (response.ok) { resetForm(); await load(); }
    setBusy(false);
  }

  async function deleteDraft(item: Challenge) {
    if (!window.confirm(t("challenge.deleteConfirm").replace("{title}", item.title))) return;
    setBusy(true); setNotice(null);
    const response = await fetch(`/api/challenges/${item.id}`, { method: "DELETE" });
    const data = await response.json() as { error?: string };
    setNotice(response.ok ? t("challenge.deleted") : data.error ?? t("challenge.actionError"));
    if (response.ok) { if (editingId === item.id) resetForm(); await load(); }
    setBusy(false);
  }
  function nextFormStep() {
    if (formStep === 1 && (!organizationId || (reviewMode === "independent" && !reviewerOrganizationId))) { setNotice(t("challenge.stepOneRequired")); return; }
  ...
  return <div id="workspace-main" tabIndex={-1} className="workspace-product-content">
    ...
    {businessMemberships.length > 0 && <section ref={builderRef} className={editingId ? "app-panel challenge-builder challenge-wizard editing-draft" : "app-panel challenge-builder challenge-wizard"}><div><span className="panel-kicker">{editingId ? t("challenge.editKicker") : t("challenge.new")}</span><h2>{editingId ? t("challenge.editTitle") : t("challenge.createBrief")}</h2><p>{editingId ? t("challenge.editDescription") : t("challenge.createDescription")}</p><ol className="wizard-steps" aria-label={t("challenge.wizardProgress")}>{[t("challenge.stepBasics"), t("challenge.stepBrief"), t("challenge.stepReward")].map((label, index) => <li className={formStep === index + 1 ? "active" : formStep > index + 1 ? "complete" : ""} key={label}><span>{formStep > index + 1 ? "✓" : index + 1}</span><strong>{label}</strong></li>)}</ol><p className="wizard-helper">{formStep === 1 ? t("challenge.stepOneHelper") : formStep === 2 ? t("challenge.stepTwoHelper") : copy.fundingHint}</p></div>
      <div className="stack-form">
        {formStep === 1 && <><label>{t("challenge.organizationLabel")}<select value={organizationId} disabled={Boolean(editingId)} onChange={(event) => setOrganizationId(event.target.value)}>{businessMemberships.map((item) => <option value={item.organization_id} key={item.id}>{item.organization_name}</option>)}</select>{editingId && <small className="field-hint">{t("challenge.ownerLocked")}</small>}</label><fieldset className="review-mode-fieldset"><legend>{t("challenge.reviewerLabel")}</legend><div className="review-mode-options" role="radiogroup" aria-label={t("challenge.reviewerLabel")}><label aria-label={t("challenge.reviewSelf")} className={reviewMode === "self" ? "review-mode-option active" : "review-mode-option"}><input type="radio" name="review-mode" value="self" checked={reviewMode === "self"} onChange={() => setReviewMode("self")} /><span><strong>{t("challenge.reviewSelf")}</strong><small>{t("challenge.reviewSelfDescription")}</small></span></label><label aria-label={t("challenge.reviewIndependent")} className={reviewMode === "independent" ? "review-mode-option active" : "review-mode-option"}><input type="radio" name="review-mode" value="independent" checked={reviewMode === "independent"} onChange={() => setReviewMode("independent")} /><span><strong>{t("challenge.reviewIndependent")}</strong><small>{t("challenge.reviewIndependentDescription")}</small></span></label></div>{reviewMode === "independent" && <label>{t("challenge.reviewSelect")}<select value={reviewerOrganizationId} onChange={(event) => setReviewerOrganizationId(event.target.value)}><option value="">{t("challenge.selectReviewer")}</option>{reviewerOrganizations.map((item) => <option value={item.id} key={item.id}>{item.name} · {item.kind === "business" ? t("challenge.businessOrganization") : t("challenge.universityOrganization")} · {translateStatus(t, item.verification_status)}</option>)}</select></label>}</fieldset></>}
        {formStep === 2 && <div className="challenge-content-form"><div className="challenge-form-main"><label>{t("challenge.titleLabel")}<input value={title} onChange={(event) => setTitle(event.target.value)} placeholder={t("challenge.titlePlaceholder")} /></label><label>{t("challenge.summaryLabel")}<textarea value={brief} onChange={(event) => setBrief(event.target.value)} placeholder={t("challenge.summaryPlaceholder")} /></label><label>{t("challenge.contextLabel")}<textarea value={context} onChange={(event) => setContext(event.target.value)} placeholder={t("challenge.contextPlaceholder")} /></label><label>{t("challenge.objectivesLabel")}<textarea value={objectives} onChange={(event) => setObjectives(event.target.value)} placeholder={t("challenge.listPlaceholder")} /><small>{t("challenge.listHelper")}</small></label><label>{t("challenge.deliverablesLabel")}<textarea value={deliverables} onChange={(event) => setDeliverables(event.target.value)} placeholder={t("challenge.listPlaceholder")} /><small>{t("challenge.listHelper")}</small></label><label>{t("challenge.constraintsLabel")}<textarea value={constraints} onChange={(event) => setConstraints(event.target.value)} placeholder={t("challenge.listPlaceholder")} /><small>{t("challenge.listHelper")}</small></label><div className="challenge-form-two-column"><label>{t("challenge.timelineLabel")}<input value={timeline} onChange={(event) => setTimeline(event.target.value)} placeholder={t("challenge.timelinePlaceholder")} /></label><label>{t("challenge.skillsLabel")}<input value={skills} onChange={(event) => setSkills(event.target.value)} placeholder={t("challenge.skillsPlaceholder")} /></label></div><label>{t("challenge.outcomeLabel")}<input value={reward} onChange={(event) => setReward(event.target.value)} placeholder={t("challenge.rewardPlaceholder")} /></label></div><aside className="challenge-content-preview"><span className="panel-kicker">{t("challenge.previewKicker")}</span><h3>{title.trim() || t("challenge.previewTitle")}</h3><p>{brief.trim() || t("challenge.previewSummary")}</p>{context.trim() && <section><strong>{t("challenge.contextLabel")}</strong><p>{context}</p></section>}{parseLines(objectives).length > 0 && <section><strong>{t("challenge.objectivesLabel")}</strong><ul>{parseLines(objectives).map((item) => <li key={item}>{item}</li>)}</ul></section>}{parseLines(deliverables).length > 0 && <section><strong>{t("challenge.deliverablesLabel")}</strong><ul>{parseLines(deliverables).map((item) => <li key={item}>{item}</li>)}</ul></section>}<div className="entity-tags">{skills.split(",").map((item) => item.trim()).filter(Boolean).slice(0, 8).map((item) => <span key={item}>{item}</span>)}</div></aside></div>}
        {formStep === 3 && <><label>{t("challenge.accessLabel")}<select value={accessType} onChange={(event) => setAccessType(event.target.value as "" | "public" | "invite_only")}><option value="">{t("challenge.selectAccess")}</option><option value="public">{t("challenge.publicOption")}</option><option value="invite_only">{t("challenge.inviteOption")}</option></select></label>{editingFinancialLocked && <p className="app-notice draft-financial-lock">{t("challenge.financialLocked")}</p>}<label>{t("challenge.rewardType")}<select value={rewardType} disabled={editingFinancialLocked} onChange={(event) => { const type = event.target.value as "" | "usdc" | "sol" | "badge"; setRewardType(type); if (type === "badge") setRewardAmount(""); }}><option value="">{t("challenge.selectRewardType")}</option><option value="usdc">{t("challenge.rewardUsdc")}</option><option value="sol">{copy.solOption}</option><option value="badge">{t("challenge.rewardBadge")}</option></select></label>
          {(rewardType === "usdc" || rewardType === "sol") && <div className="invoice-form-grid"><label>{copy.rewardAmount} ({rewardType === "sol" ? copy.sol : copy.usdc})<input inputMode="decimal" disabled={editingFinancialLocked} value={rewardAmount} onChange={(event) => setRewardAmount(event.target.value)} placeholder="10" /></label><label>{t("challenge.rewardSlots")}<input type="number" min="1" max="100" disabled={editingFinancialLocked} value={rewardSlots} onChange={(event) => setRewardSlots(event.target.value)} /></label></div>}
          {rewardType === "badge" && <><label>{t("challenge.badgeName")}<input value={badgeNameValue} disabled={editingFinancialLocked} onChange={(event) => setBadgeNameValue(event.target.value)} placeholder={t("challenge.badgeName")} /></label><label>{t("challenge.badgeDescription")}<textarea value={badgeDescription} disabled={editingFinancialLocked} onChange={(event) => setBadgeDescription(event.target.value)} placeholder={t("challenge.badgeDescription")} /></label><label>{t("challenge.rewardSlots")}<input type="number" min="1" max="100" disabled={editingFinancialLocked} value={rewardSlots} onChange={(event) => setRewardSlots(event.target.value)} /></label><p className="field-hint">{copy.badgeBond}: {copy.sol} ({vi ? "chỉ hoàn khi chưa giải ngân" : "refundable before a payout"}).</p></>}
        ...
        <div className="wizard-actions">{editingId && <button className="button button-text" type="button" disabled={busy} onClick={resetForm}>{t("common.cancel")}</button>}{formStep > 1 && <button className="button button-secondary" type="button" disabled={busy} onClick={() => setFormStep((current) => (current - 1) as 1 | 2 | 3)}>{t("common.back")}</button>}{formStep < 3 ? <button className="button button-primary" type="button" disabled={busy} onClick={nextFormStep}>{t("common.continue")}</button> : <button className="button button-primary" type="button" disabled={busy || !organizationId || (reviewMode === "independent" && !reviewerOrganizationId) || !accessType || !rewardType} onClick={() => void saveDraft()}>{editingId ? t("challenge.saveChanges") : t("challenge.create")}</button>}</div>
      </div></section>}
    <section className="challenge-list">{challenges.map((item) => { const content = challengeContent(item); const deleteBlocked = item.fund_status === "funded" || ["checking", "pending_finalization"].includes(item.fund_verification_state || ""); return <article className={item.can_manage ? "challenge-card challenge-card-managed" : "challenge-card"} key={item.id}>
      <div className="challenge-card-summary"><div className="entity-top"><span>{item.organization_name}</span><b>{translateStatus(t, item.status)} · {item.access_type === "public" ? t("challenge.public") : t("challenge.inviteOnly")}</b></div><div className={`challenge-review-provenance ${item.review_mode === "self" ? "internal" : "independent"}`}><span>{item.review_mode === "self" ? t("challenge.reviewInternal") : t("challenge.reviewIndependentLabel")}</span><strong>{item.reviewer_organization_name || "—"}</strong></div><h2>{item.title}</h2><p>{content.summary}</p><div className="entity-tags">{parseSkills(item.skills_json).map((skill) => <span key={skill}>{skill}</span>)}</div>
      <dl className="challenge-summary-meta"><div><dt>{t("challenge.deadlineShort")}</dt><dd>{content.timeline || t("challenge.notSpecified")}</dd></div><div><dt>{t("challenge.rewardShort")}</dt><dd>{item.reward_type === "badge" ? `${t("challenge.rewardBadge")}: ${badgeName(item)}` : `${item.reward_amount_usdc ?? "0"} ${item.reward_type === "sol" ? copy.sol : copy.usdc}`}</dd></div><div><dt>{t("challenge.participationShort")}</dt><dd>{translateStatus(t, item.participation_state ?? "not_joined")}</dd></div></dl><div className="challenge-card-links"><Link className="challenge-details-link" href={`/app/challenges/${item.id}`}>{t("challenge.viewDetails")} <span aria-hidden="true">→</span></Link>{Boolean(item.can_manage) && item.status === "draft" && <button className="button button-secondary" type="button" disabled={busy} onClick={() => editDraft(item)}>{t("challenge.editDraft")}</button>}</div></div>
      {item.can_manage && (item.escrow_address || (["sol","usdc"].includes(item.reward_type) && !item.fund_status)) ? <div className="challenge-actions"><Link className="button button-primary" href={"/app/escrow?challenge="+item.id}>{vi?"Quản lý quỹ thưởng":"Manage reward fund"}</Link>{item.status==="published" && item.access_type==="invite_only" && <><input value={targetWallet} onChange={e=>setTargetWallet(e.target.value)} placeholder={t("challenge.targetWallet")}/><button className="button button-secondary" onClick={()=>void invite(item.id)}>{t("challenge.createInvitation")}</button></>}</div> : item.can_manage ? <div className="challenge-actions challenge-manager-actions"><div className="challenge-management-heading"><span className="panel-kicker">{t("challenge.managementKicker")}</span><strong>{t("challenge.managementTitle")}</strong></div>
        {item.status !== "closed" && <label className="challenge-access-control">{t("challenge.accessControlLabel")}<select aria-label={`${t("challenge.accessControlLabel")} ${item.title}`} value={item.access_type} disabled={busy} onChange={(event) => void updateAccess(item.id, event.target.value as "public" | "invite_only")}><option value="public">{t("challenge.managerPublic")}</option><option value="invite_only">{t("challenge.managerInviteOnly")}</option></select></label>}
        ...
        {item.status === "draft" && <div className="draft-danger-zone">{deleteBlocked && <p>{item.fund_status === "funded" ? t("challenge.deleteRefundFirst") : t("challenge.deleteVerificationPending")}</p>}<button className="button button-danger-text" type="button" disabled={busy || deleteBlocked} onClick={() => void deleteDraft(item)}>{t("challenge.deleteDraft")}</button></div>}
      </div> : item.participation_id ? <div className="challenge-actions"><Link className="button button-dark" href="/app/submissions">{t("challenge.openSubmission")}</Link></div> : item.status === "published" && item.access_type === "public" ? <div className="challenge-actions"><button className="button button-primary" type="button" disabled={busy} onClick={() => void join(item.id)}>{t("challenge.join")}</button></div> : null}
    </article>; })}</section>
