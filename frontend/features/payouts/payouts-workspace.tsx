...
export function PayoutsWorkspace() {
  ...
  async function load() { const response = await apiFetch("/api/payouts", { cache: "no-store" }); if (response.ok) setItems(((await response.json()) as { payouts: Payout[] }).payouts); }
  useEffect(() => {
    let active = true;
    apiFetch("/api/payouts", { cache: "no-store" })
  ...
  async function release(item: Payout) {
    ...
    try {
    const response = await apiFetch("/api/payouts", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ submissionId: item.submission_id }) });
  ...
  return <div id="workspace-main" tabIndex={-1} className="workspace-product-content"><a className="profile-menu-link" href="/app/escrow">{t("nav.escrow")} →</a>
    <div className="app-welcome"><div><span>{vi ? "GIẢI NGÂN CÓ PHÊ DUYỆT" : "HUMAN-APPROVED PAYOUTS"}</span><h1>{vi ? "Người duyệt quyết định. Vault thực hiện." : "A reviewer decides. The vault executes."}</h1><p>{vi ? "AI không tự chuyển tiền. Sau khi bài được con người phê duyệt, reviewer ghi kết quả, phân bổ phần thưởng và sinh viên nhận về ví." : "AI never moves money. Once a human approves a submission, the active reviewer records the result and allocates the award; the student then claims it."}</p></div><div className="identity-card"><small>{vi ? "BÀI ĐỦ ĐIỀU KIỆN" : "ELIGIBLE SUBMISSIONS"}</small><strong className="metric-number">{items.length}</strong><b>{paidCount} {vi ? "đã giải ngân" : "paid out"}</b></div></div>
    ...
    {items.length === 0 ? <section className="app-panel empty-product"><h2>{t("payout.empty")}</h2><p>{t("payout.emptyDescription")}</p></section> : <section className="payout-list">{items.map((item) => { const asset = item.reward_asset === "sol" ? "SOL" : "USDC"; const vaultReady = item.fund_status === "funded"; return <article className={`payout-card ${item.payout_status ?? "pending"}`} key={item.submission_id}>
      <div className="entity-top"><span>{item.organization_name}</span><b>{item.payout_status === "paid" ? t("invoice.paid") : item.escrow_id ? (item.chain_verified ? (item.payout_status==="allocated" ? (vi?"Chờ sinh viên nhận":"Awaiting claim") : (vi?"Chờ bước tiếp theo":"Next step pending")) : (vi?"Chưa đọc được quỹ":"Fund unavailable")) : vaultReady ? (vi ? "Sẵn sàng giải ngân" : "Ready to release") : (vi ? "Quỹ chưa sẵn sàng" : "Vault not ready")}</b></div><h2>{item.challenge_title}</h2><p>{item.student_name || shortWallet(item.recipient_wallet)}</p><div className="payout-amount"><strong>{item.reward_amount_usdc}</strong><span>{asset}</span></div>
      ...
      {item.payout_status === "paid" ? <a className="chain-proof-link" href={item.payment_tx ? `https://explorer.solana.com/tx/${item.payment_tx}?cluster=devnet` : `/app/escrow?challenge=${encodeURIComponent(item.challenge_id)}#submission-${item.submission_id}`} target="_blank" rel="noreferrer">{vi ? "Xem bằng chứng giao dịch" : "View transaction proof"}</a> : item.escrow_id ? <div className="payout-verify"><a className="button button-primary" href={`/app/escrow?challenge=${encodeURIComponent(item.challenge_id)}#submission-${item.submission_id}`}>{vi ? "Tiếp tục xử lý phần thưởng" : "Continue reward workflow"}</a><p className="field-hint">{vi ? "Challenge này dùng escrow program. Reviewer xác nhận kết quả và người nhận claim thưởng trong luồng escrow." : "This challenge uses the escrow program. The reviewer confirms the result and the recipient claims the reward in the escrow workflow."}</p></div> : <div className="payout-verify"><button className="button button-primary" type="button" disabled={busy || !vaultReady} onClick={() => void release(item)}>{vi ? "Xác nhận giải ngân" : "Confirm release"}</button>{!vaultReady && <p className="field-hint">{vi ? "Challenge này chưa có quỹ Devnet đã xác minh." : "This challenge has no verified Devnet vault yet."}</p>}</div>}
    </article>; })}</section>}
