...
export function PayoutsWorkspace() {
  ...
  return <div id="workspace-main" tabIndex={-1} className="workspace-product-content"><a className="profile-menu-link" href="/app/escrow">{t("nav.escrow")} →</a>
    ...
    {items.length === 0 ? <section className="app-panel empty-product"><h2>{t("payout.empty")}</h2><p>{t("payout.emptyDescription")}</p></section> : <section className="payout-list">{items.map((item) => { const asset = item.reward_asset === "sol" ? "SOL" : "USDC"; const vaultReady = item.fund_status === "funded"; return <article className={`payout-card ${item.payout_status ?? "pending"}`} key={item.submission_id}>
      <div className="entity-top"><span>{item.organization_name}</span><b>{item.payout_status === "paid" ? t("invoice.paid") : item.escrow_id ? (vi ? "Escrow program" : "Escrow program") : vaultReady ? (vi ? "Sẵn sàng giải ngân" : "Ready to release") : (vi ? "Quỹ chưa sẵn sàng" : "Vault not ready")}</b></div><h2>{item.challenge_title}</h2><p>{item.student_name || shortWallet(item.recipient_wallet)}</p><div className="payout-amount"><strong>{item.reward_amount_usdc}</strong><span>{asset}</span></div>
      ...
      {item.payout_status === "paid" ? <a className="chain-proof-link" href={`https://explorer.solana.com/tx/${item.payment_tx}?cluster=devnet`} target="_blank" rel="noreferrer">{vi ? "Xem bằng chứng giao dịch" : "View transaction proof"}</a> : item.escrow_id ? <div className="payout-verify"><a className="button button-primary" href={`/app/escrow?challenge=${encodeURIComponent(item.challenge_id)}`}>{vi ? "Mở quy trình escrow" : "Open escrow workflow"}</a><p className="field-hint">{vi ? "Challenge này dùng escrow program. Reviewer xác nhận kết quả và người nhận claim thưởng trong luồng escrow." : "This challenge uses the escrow program. The reviewer confirms the result and the recipient claims the reward in the escrow workflow."}</p></div> : <div className="payout-verify"><button className="button button-primary" type="button" disabled={busy || !vaultReady} onClick={() => void release(item)}>{vi ? "Xác nhận giải ngân" : "Confirm release"}</button>{!vaultReady && <p className="field-hint">{vi ? "Challenge này chưa có quỹ Devnet đã xác minh." : "This challenge has no verified Devnet vault yet."}</p>}</div>}
    </article>; })}</section>}
