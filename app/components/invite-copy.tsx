...
type ChallengeInvite = { title: string; brief: string; reward: string; organization_name: string };
...
export function ChallengeInviteCopy({ token, row, valid, signedIn }: { token: string; row: ChallengeInvite | null; valid: boolean; signedIn: boolean }) {
  const { t } = useLanguage();
  return <main className="auth-page"><header className="auth-header page-shell"><Link className="wordmark" href="/"><span className="wordmark-mark">S</span><span>SkillBridge</span></Link><div className="topbar-actions"><LanguageSwitcher /><Link className="text-link" href="/">{t("common.home")}</Link></div></header><section className="join-card"><span>{t("invite.challengeKicker")}</span><h1>{valid && row ? row.title : t("invite.invalidChallenge")}</h1>{valid && row && <><p><strong>{row.organization_name}</strong><br />{row.brief}</p><div className="invite-reward"><small>{t("invite.opportunity")}</small><strong>{row.reward}</strong></div>{signedIn ? <AcceptChallenge token={token} /> : <Link className="button button-primary" href={`/auth?returnTo=${encodeURIComponent(`/challenge/${token}`)}`}>{t("invite.loginChallenge")}</Link>}</>}</section></main>;
}
