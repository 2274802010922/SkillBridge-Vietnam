  ...
  return <div className="workspace-product-content">
    ...
    <section className="challenge-list">{challenges.map((item) => <article className="challenge-card" key={item.id}>
      ...
      {item.can_manage ? <div className="challenge-actions challenge-manager-actions">
        ...
        {item.status === "draft" && <button className="button button-primary" type="button" disabled={busy || item.fund_status !== "funded"} title={item.fund_status !== "funded" ? copy.publishFirst : undefined} onClick={() => void publish(item.id)}>{t("challenge.publish")}</button>}
