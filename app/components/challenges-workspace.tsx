...
type Challenge = {
  id: string; organization_id: string; organization_name: string; reviewer_organization_id: string | null; reviewer_organization_name: string | null; reviewer_organization_kind: "business" | "university" | null; review_mode: "self" | "independent" | null; title: string; brief: string; skills_json: string;
...
export function ChallengesWorkspace({ memberships, reviewerOrganizations }: { memberships: Membership[]; reviewerOrganizations: ReviewerOrganization[] }) {
  ...
  async function create() {
    ...
    const response = await fetch("/api/challenges", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ organizationId, reviewerOrganizationId: reviewMode === "independent" ? reviewerOrganizationId : organizationId, reviewMode, title, brief, skills: skills.split(","), reward, rewardType: rewardType || undefined, badgeName: badgeNameValue || undefined, badgeDescription: badgeDescription || undefined, rewardSlots: Number(rewardSlots) || 1, minimumScore, rewardAmountUsdc: rewardAmount || undefined, accessType }) });
