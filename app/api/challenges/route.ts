  ...
  try {
    ...
    const rows = await env.DB.prepare(`
      SELECT DISTINCT c.id, c.organization_id, o.name AS organization_name,
        c.title, c.brief, c.skills_json, c.rubric_json, c.reward, c.access_type, c.status,
...
export async function POST(request: Request) {
  try {
    ...
    const body = (await request.json()) as {
      ...
      reward?: string; closesAt?: string | null; accessType?: ChallengeAccessType;
    };
    ...
    await env.DB.batch([env.DB.prepare(`
      INSERT INTO challenges
        (id, organization_id, reviewer_organization_id, created_by_user_id, title, brief, skills_json, rubric_json, reward, access_type, closes_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    return Response.json({ challenge: { id, organizationId: body.organizationId, reviewerOrganizationId: body.reviewerOrganizationId, title, brief, skills, rubric: RUBRIC, reward, accessType, status: "draft" } }, { status: 201 });
  } catch (error) { return jsonError(error); }
}
